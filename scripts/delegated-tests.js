...
async function main() {
  ...
  console.log("Delegating to account 1")
  await lendingPool.delegate(accounts[1].address, hre.ethers.utils.parseEther('2'))
  console.log("Borrowing from account 1, delegated by account 0")

  const lendingPool1 = lendingPool.connect(accounts[1])
  await lendingPool1.borrow(hre.ethers.utils.parseEther("1"), accounts[0].address)
  console.log("User balance After Borrow:", (await provider.getBalance(accounts[0].address)).toString())
  console.log("Delegatee balance After Borrow:", (await provider.getBalance(accounts[1].address)).toString())
  console.log("Contract balance After Borrow:", (await provider.getBalance(lendingPool.address)).toString(), "\n")

  await lendingPool1.repay(accounts[0].address, { value: hre.ethers.utils.parseEther("9") })
  console.log("Repayed 9")
  console.log("User balance After Repay:", (await provider.getBalance(accounts[0].address)).toString())
  console.log("Delegatee balance After Repay:", (await provider.getBalance(accounts[1].address)).toString())
